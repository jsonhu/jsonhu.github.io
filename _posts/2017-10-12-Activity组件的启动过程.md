...
在setInitialState方法中将r赋值给mStartActivity,所以这个mStartActivity对应的是即将被启动的MainActivity的ActivityRecord,因为不需要知道result返回的结果所以resultTo为null,此时的TaskRecord并没有创建所以为null,参数mAddingToTask初始化为false,表示是否为Activity创建一个专属的任务栈.在点击Launcher桌面的时候知道这个intent flag为FLAG_ACTIVITY_NEW_TASK ,它的值为1,所以进入if判断中,newTask置为true,mTartgetStack表示我们目标Activity的任务栈,在setTaskFromReuseOrCreateNewTask里创建一个新的ActivityStack赋值给他,接着调用了ActivityStack的startActivityLocked方法:
