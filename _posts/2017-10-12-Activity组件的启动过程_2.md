...
```
1.为Activity创建上下文环境ContextImpl
2.通过classloader创建Activity对象
3.通过makeApplication找到清单文件中是否存在自定义的Application，并创建，和Activity创建方式一样，并先后调用了Application的attach和onCreate函数
4.callActivityOnCreate和activity.performStart();先后调用Activity的onCreate和onStart方法，
5.回到handleLaunchActivity中，继续调用了handleResumeActivity触发Activity的onResume方法。
